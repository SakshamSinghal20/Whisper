...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("┌─────────────────────────────────────────┐");
    println!("│  Whisper — Silent Payments Client Demo   │");
    println!("└─────────────────────────────────────────┘\n");
    ...
    match client.get_status().await {
        Ok(status) => {
            println!("  ✓ Server: {} ({})", status.status, status.network);
        ...
        Err(e) => {
            ...
            eprintln!("    Make sure the server is running at {}\n", client.base_url);
            return Ok(());
        }
    }
    
    // 4. Scan for payments
    println!("  Scanning blocks 0-100...");
    ...
    match client.scan_range(0, 100, &inputs).await {
        Ok(results) => {
            if results.is_empty() {
